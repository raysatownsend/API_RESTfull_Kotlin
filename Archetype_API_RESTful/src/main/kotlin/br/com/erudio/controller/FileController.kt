...
@Tag(name = "Download & Upload File Endpoint")
@RestController
...
class FileController {
    ...
    fun downloadMultipleFiles(@RequestParam("files") files: Array<MultipartFile>): List<UploadFileResponseVO> {
        val uploadFileVOs = arrayListOf<UploadFileResponseVO>()
        for (file in files) {
            var uploadFileResponseVO: UploadFileResponseVO = uploadFile(file)
            uploadFileVOs.add(uploadFileResponseVO)
        }
    ...
    fun downloadFile(@PathVariable fileName: String, request: HttpServletRequest): ResponseEntity<Resource> {
        ...
        try {
            ...
            logger.info("File type is incorrect.")
        }
